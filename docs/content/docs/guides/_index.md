...
for: the install, the claim that plays sound to an output, and the
...
`ResourceSlice`, the inventory the scheduler reads: one device for
each physical output, named like `card0-pcm3`, with the facts about
it as attributes, such as the sink a stream targets and the
monitor-pairing identity `monitor.liken.sh/id`.
...
class can also be specific, its selector baked in so claims carry
