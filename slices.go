...
// questions about one output that cannot play.
//
...
//
// noSinkTaint says a claim on the output would deliver a sink name
// that names nothing, so the consumer would connect to PipeWire and
// play into no hardware. The effect is NoSchedule, and no consumer
// should ever tolerate it. This is what makes a claim ahead of a
// monitor park instead of loop: with only the NoExecute taint, a
// consumer that tolerated it would be scheduled onto an output with
// no sink, fail in prepare, get evicted when the toleration ran out,
// and be scheduled again. An untolerated NoSchedule taint holds the
// pod Unschedulable until the output can really play.
const (
...
// two taints, never the device, because deleting a device that a
...
func sliceDevices(outputs []alsaOutput, sinks map[pcmAddress]string) []SliceDevice {
	...
	for _, output := range outputs {
		...
		if hasSink && len(sink) <= maxAttributeLength {
			// A name too long for an attribute still reaches the
			// consumer, because the delivery reads the current name from
			// PipeWire rather than from the slice. Publishing a truncated
			// one would give a selector a name that names nothing.
			device.Attributes["sinkName"] = AttrString(sink)
		}
		if output.Monitor {
		...
		// HDMI output plays only when a monitor answers on it. Both
		// taints carry the same condition and different keys, because a
		// consumer tolerates one of them and never the other.
		if !hasSink || (output.HDMI && !output.Monitor) {
			device.Taints = []DeviceTaint{
