...
func TestRunDispatchesCapture(t *testing.T) {
	var stdout, stderr strings.Builder
	err := run(context.Background(), []string{"capture", "room", "--format", "mp3"}, &stdout, &stderr)
	if err == nil {
...
func TestArgAt(t *testing.T) {
	positional := []string{"capture", "room"}
	if got := argAt(positional, 1); got != "room" {
		t.Fatalf("argAt(1) = %q, want room", got)
	}
